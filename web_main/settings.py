...
# 프록시 서버(Nginx/Load Balancer)로부터 HTTPS 프로토콜을 신뢰하도록 설정
# SECURE_PROXY_SSL_HEADER = ('HTTP_X_FORWARDED_PROTO', 'https')

# # HTTPS를 강제하려면 True로 설정 (HTTP 요청을 HTTPS로 리다이렉트)
# SECURE_SSL_REDIRECT = True

# # 세션 쿠키가 HTTPS 연결을 통해서만 전송되도록 설정
# SESSION_COOKIE_SECURE = True

# # CSRF 쿠키가 HTTPS 연결을 통해서만 전송되도록 설정
# CSRF_COOKIE_SECURE = True

# # HTTP Strict Transport Security (HSTS) 설정 (브라우저가 항상 HTTPS로만 접속하도록 강제)
